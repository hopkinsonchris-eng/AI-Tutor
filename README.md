...
Five courses are hand-built from the board's own PDF and ship with a written and examiner-checked kit in every room: **OCR Geography H481**, **Edexcel Business 9BS0**, **Edexcel Politics 9PL0**, **Edexcel Mathematics 9MA0** (19 rooms, 299 questions re-solved by a second model, worked examples with cues, formulae sheets checked against Pearson's own booklet) and **AQA Combined Science 8464**. Every other GCSE and A level in the catalogue (97 qualifications across AQA, Pearson Edexcel, OCR and Eduqas) is built on request: a student picks level, subject and board, presses **Add**, and the Worker builds the specification map from the board's own PDF while a progress bar shows the stages — then it's theirs. Built courses are held to the same validator as the hand-written ones, judged against the document before they go live, and checked against the document again every month (see *Courses*).
