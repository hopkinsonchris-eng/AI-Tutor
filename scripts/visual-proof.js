...
const SPECS = {}; for (const sp of Object.values({ ...require(path.join(ROOT, 'src', 'specs', 'ocr-h481.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-9ma0.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-4gn1.js')), ...require(path.join(ROOT, 'src', 'specs', 'aqa-8700.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-4ma1.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-1ma1.js')), ...require(path.join(ROOT, 'src', 'specs', 'aqa-8702.js')) })) SPECS[sp.id] = sp; SPECS['AQA-7402'] = biology;
...
(async () => {
  ...
  must((await page.locator('#v-campus .bldg[data-bldg]').count()) === 8 && (await page.locator('#v-campus [data-bldg-view="exam"]').count()) === 1 && (await page.locator('#v-campus [data-bldg-view="office"]').count()) === 1, 'campus: eight subject buildings, the Exam Hall and the Office');
  must((await page.locator('#qBar [data-course]').count()) === 8 && (await page.locator('#qBar [data-v="exam"]').count()) === 1 && (await page.locator('#qBar [data-v="office"]').count()) === 1 && !(await page.locator('nav.bottom').isVisible()), '1280px: the quick bar carries every course, the Exam Hall and the Office; no bottom bar');
  ...
  must(/data-act="signout"|id="signOut"/.test(await page.locator('#v-office').innerHTML()) && (await page.locator('#v-office .ccard').count()) === 8, 'the Office: Add or change courses, the account with Sign out, the eight courses with pins');
