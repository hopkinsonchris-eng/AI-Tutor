...
const FAMILIES = {
  ...
  language: {
    ...
    rules: [
      ...
      'Topics are the specification’s themes and sub-themes (for example "Identity and culture — family and relationships"); key ideas within a topic are the vocabulary fields and the grammar the theme is used to test. The papers’ own descriptions — listening, reading, writing and speaking as sections of the document, with their timings, task types and mark grids — are never topics: they belong in the component’s `sections`, and a component that examines every theme is marked `coversAll`. A room built from them produces cards about how long a paper lasts, which is not knowledge.',
      'A grammar topic per tier or level lists the structures the specification requires (tenses, agreement, negation, subjunctive where relevant) as key ideas with content naming the forms. Where the document runs further sub-headings onto the end of a numbered grammar topic with no code of their own (for example "Adjectival phrases", "Adverbial phrases" or "Prepositions" printed after "Verb phrases", still under its number), fold them in as extra key ideas of that same topic rather than giving them a topic of their own: a topic with no code of its own to give its ideas either falls short of the two ideas required or has to duplicate its own topic code across them, and the validator refuses both.',
