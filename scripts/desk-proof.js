...
(async () => {
  ...
  const deskFacts = await page.evaluate(() => { const g = s => document.querySelector('svg.desk ' + s); return { objects: document.querySelectorAll('svg.desk .dobj').length, video: g('[aria-label^="Videos: resume"]') && g('[aria-label^="Videos: resume"]').getAttribute('aria-label'), note: g('[data-postit="note"]') && g('[data-postit="note"]').textContent.slice(0, 40), mine: g('[data-postit="mine"]') && g('[data-postit="mine"]').textContent.slice(0, 30), coach: g('[data-station="coach"][aria-label^="From Coach: Hard questions"]') ? 'weekly note on the pink post-it' : 'NO WEEKLY NOTE', photo: g('[aria-label^="Photos of your notes"]') && !!g('[aria-label^="Photos of your notes"] image'), cards: g('[data-station="cards"][aria-label^="Flash cards:"]').getAttribute('aria-label'), flag: !!g('.dflag'), plant: g('[data-plant]').dataset.plant, piles: [...document.querySelectorAll('svg.desk [data-pile]')].map(p => p.dataset.sheets).join('/'), planner: g('[data-station="planner"]').textContent }; });
  ...
  console.log(deskFacts.objects >= 14 && /resume .* at 0:47/.test(deskFacts.video || '') && deskFacts.note && deskFacts.mine && deskFacts.coach !== 'NO WEEKLY NOTE' && deskFacts.photo && deskFacts.flag && deskFacts.plant === 'Fluent' && keyboardOpens && requests === 0 && emptyFacts.prompt && emptyFacts.note && emptyFacts.plant === 'Unassessed' && !smallHits.length && captionsHidden ? 'DESK PROOF: criteria 1, 2, 4, 5, 6, 7, 9, 11 and 12 seen in the browser, with the weekly note' : 'DESK PROOF: FAILED');
