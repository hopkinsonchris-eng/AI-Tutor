...
(async()=>{await sleep(40);const G=sb;G0=sb;
 ...
 G.UI.station='lesson';G.renderRooms();ok('X3 the three piles hold the bank by difficulty',/data-pile="1" data-sheets="4"/.test(svg())&&/data-pile="2" data-sheets="4"/.test(svg())&&/data-pile="3" data-sheets="4"/.test(svg())&&/Standard · 4/.test(svg())&&/Exam · 4/.test(svg())&&/Hard · 4/.test(svg()));
 ...
 ok('X4 the card box shows the deck size and a red flag with the due count',new RegExp(`data-station="cards" aria-label="Cards: ${roomCards.length} in the deck, ${dueNow} due"`).test(svg())&&/class="dflag"/.test(svg())&&new RegExp(`>${dueNow} due<`).test(svg())&&new RegExp(`Cards · ${roomCards.length}<`).test(svg()),String(roomCards.length)+' '+String(dueNow));
 for(const [,c] of roomCards)c.due='2099-01-01';G.renderRooms();ok('X4 no flag when nothing is due (criterion 4)',!/class="dflag"/.test(svg())&&/aria-label="Cards: \d+ in the deck, nothing due"/.test(svg()));for(const [,c] of roomCards)c.due=G.TODAY;
 /* the tablet */
