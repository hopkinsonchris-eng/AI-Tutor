...
let COURSE_STATES={'OCR-H432':{status:'retracted'},'EDX-4GN1':{status:'published'}};let lastBuildReq=null;let buildPolls=0;
...
(async()=>{await sleep(40);const G=sb;G0=sb;
 ...
 G.openRoom(geoRoom,'lesson');ok('G4 the open room is marked current in the tree, and the right rail shows this room: state, cards, mistakes and links',/class="tnode cur" data-open="OCR-H481\|1\.2"/.test(reg['rail-l'].innerHTML)&&/This room/.test(reg['rail-r'].innerHTML)&&/Fluent/.test(reg['rail-r'].innerHTML)&&/data-cards="1"/.test(reg['rail-r'].innerHTML)&&/EVALUATION/.test(reg['rail-r'].innerHTML)&&/Watch and read/.test(reg['rail-r'].innerHTML)&&/bbc\.co\.uk\/bitesize\/search/.test(reg['rail-r'].innerHTML),reg['rail-r'].innerHTML.slice(0,400));
 await sleep(30);ok('G4 the rail then lists the videos the Worker found and checked — title, channel, length, why and a Pin button — and never a YouTube search link',(()=>{const h=reg['rail-r'].innerHTML;const q=videoReqs[0];return /Coasts: erosion landforms explained/.test(h)&&/Geography with Mr P · 9:12/.test(h)&&/wave-cut/.test(h)&&/data-pinvideo="abc123def45"/.test(h)&&/i\.ytimg\.com\/vi\/abc123def45/.test(h)&&!/youtube\.com\/results/.test(h)&&!/Finding videos/.test(h)&&q&&q.spec==='OCR-H481'&&q.topic==='1.2'&&q.board==='OCR'&&/Earth/.test(q.topicName)&&q.ideas.length>0&&/^1\.a /.test(q.ideas[0]);})(),reg['rail-r'].innerHTML.slice(0,600));
 await click({dataset:{pinvideo:'abc123def45'}});await sleep(30);ok('G4 Pin puts the video on the room’s desk and the rail says so',(()=>{const its=DESK.items['OCR-H481|1.2']||[];return its.some(i=>i.kind==='video'&&i.url==='https://www.youtube.com/watch?v=abc123def45'&&i.title==='Coasts: erosion landforms explained')&&/On your desk/.test(reg['rail-r'].innerHTML)&&!/data-pinvideo="abc123def45"/.test(reg['rail-r'].innerHTML);})(),JSON.stringify(DESK.items['OCR-H481|1.2']||[]).slice(0,300));DESK.items['OCR-H481|1.2']=[];
 G.openRoom('OCR-H481|1.1.1','lesson');await sleep(30);ok('G4 a room where nothing passed the checks lists no videos at all — no search link, no placeholder',(()=>{const h=reg['rail-r'].innerHTML;return /Watch and read/.test(h)&&!/class="vid"/.test(h)&&!/Finding videos/.test(h)&&!/youtube\.com/.test(h)&&videoReqs.length===2&&videoReqs[1].topic==='1.1.1';})(),reg['rail-r'].innerHTML.slice(0,400));
 G.openRoom(geoRoom,'lesson');await sleep(30);ok('G4 reopening a room asks the Worker again (it answers from its cache) and offers Pin once the desk item is gone',videoReqs.length===3&&/data-pinvideo="abc123def45"/.test(reg['rail-r'].innerHTML));
 ...
 ok('D1 the room has a Desktop tab and an empty desktop invites the first item',/data-station="desktop"/.test(reg['v-rooms'].innerHTML)&&/id="dkPhoto"/.test(reg['v-rooms'].innerHTML)&&/Nothing here yet/.test(reg['v-rooms'].innerHTML));
 ...
 ok('X2b the flash-card list keeps answers hidden until asked',(()=>{G.UI.station='cards';G.renderRooms();const V=reg['v-rooms'].innerHTML;const id=Object.keys(G.S.cards).find(k=>G.S.cards[k].node==='OCR-H481|1.2');const c=G.S.cards[id];return /Show answer/.test(V)&&!V.includes('— '+c.back.slice(0,20))&&(()=>{G.UI.revealed['cd'+id]=true;G.renderRooms();return reg['v-rooms'].innerHTML.includes('— '+c.back.slice(0,20));})();})());G.UI.station='practise';G.renderRooms();
 ...
 ok('X2 the open station is marked on the desk itself',/data-station="practise" aria-label="[^"]*" aria-pressed="true"[\s\S]*?class="dsel"/.test(svg())&&(svg().match(/aria-pressed="true"/g)||[]).length===3&&(svg().match(/class="dsel"/g)||[]).length===3,String((svg().match(/aria-pressed="true"/g)||[]).length));
