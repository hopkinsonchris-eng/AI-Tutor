...
const SPECS = {}; for (const sp of Object.values({ ...require(path.join(ROOT, 'src', 'specs', 'ocr-h481.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-9ma0.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-4gn1.js')), ...require(path.join(ROOT, 'src', 'specs', 'aqa-8700.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-4ma1.js')), ...require(path.join(ROOT, 'src', 'specs', 'edexcel-1ma1.js')), ...require(path.join(ROOT, 'src', 'specs', 'aqa-8702.js')), ...require(path.join(ROOT, 'src', 'specs', 'aqa-8035.js')), ...require(path.join(ROOT, 'src', 'specs', 'aqa-8300.js')) })) SPECS[sp.id] = sp; SPECS['AQA-7402'] = biology;
const setup = { student: 'Matthew', examYear: 2028, subjects: [{ specId: 'OCR-H481', options: { landscape: '1.1.1', globalSystems: '2.2.1', globalGovernance: '2.2.4', debates: ['3.1', '3.5'] } }, { specId: 'EDX-9MA0', options: {} }, { specId: 'AQA-7402', options: {} }, { specId: 'EDX-4GN1', options: {} }, { specId: 'EDX-4MA1', options: {} }, { specId: 'AQA-8700', options: {} }, { specId: 'EDX-1MA1', options: { tier: 'higher' } }, { specId: 'AQA-8702', options: { shakespeare: 'shakespeare-macbeth', novel: 'novel-a-christmas-carol', modernText: 'modern-an-inspector-calls', cluster: 'poetry-power-and-conflict' } }, { specId: 'AQA-8035', options: { livingWorld: '3.1.2.3', landscapes: ['3.1.3.2', '3.1.3.3'], resource: '3.2.3.3' } }, { specId: 'AQA-8300', options: { tier: 'higher' } }] };
...
kit.extras = [{ kind: 'practical', title: 'Required practical 1: microscopy — method sheet', items: ['Prepare a temporary mount: a drop of water, the specimen, a coverslip lowered at an angle to avoid air bubbles.', 'Independent variable: specimen; dependent: image; control: magnification and stain.', 'Skills assessed: AT d, AT e, WS 2.1.'] }, { kind: 'extended', title: 'Six-mark model answer: testing a solution for a non-reducing sugar (6 marks)', question: 'Describe how you would show that a solution contains a non-reducing sugar. (6 marks)',
  expected: 'The examiner wants the two Benedict’s tests in order with the reason for each: a negative result first, then acid hydrolysis, neutralisation and a positive second test, with the colour change named and linked to what it shows. Weaker answers describe one Benedict’s test and forget the neutralising step.',
  levels: [{ level: 1, marks: '1–2', answer: 'I would add Benedict’s solution to the sample and heat it in a water bath. If it stays blue there is no reducing sugar present.' },
    { level: 2, marks: '3–4', answer: 'I would add Benedict’s solution to the sample and heat it in a water bath. If it stays blue there is no reducing sugar present. I would then boil a fresh sample with dilute hydrochloric acid to hydrolyse any non-reducing sugar into its monosaccharides. I would repeat the Benedict’s test on this sample and look for a brick-red precipitate.' },
    { level: 3, marks: '5–6', answer: 'I would add Benedict’s solution to the sample and heat it in a water bath. If it stays blue there is no reducing sugar present. I would then boil a fresh sample with dilute hydrochloric acid to hydrolyse any non-reducing sugar into its monosaccharides. Before testing again I would neutralise the acid with sodium hydrogencarbonate, because Benedict’s solution does not work in acidic conditions. I would repeat the Benedict’s test on this sample and look for a brick-red precipitate. A brick-red precipitate now, after a negative first test, shows that a non-reducing sugar such as sucrose was present and has been hydrolysed into reducing sugars.' }] }];
...
(async () => {
  ...
  must((await page.locator('#v-campus .bldg[data-bldg]').count()) === 10 && (await page.locator('#v-campus [data-bldg-view="exam"]').count()) === 1 && (await page.locator('#v-campus [data-bldg-view="office"]').count()) === 1, 'campus: ten subject buildings, the Exam Hall and the Office');
  must((await page.locator('#qBar [data-course]').count()) === 10 && (await page.locator('#qBar [data-v="exam"]').count()) === 1 && (await page.locator('#qBar [data-v="office"]').count()) === 1 && !(await page.locator('nav.bottom').isVisible()), '1280px: the quick bar carries every course, the Exam Hall and the Office; no bottom bar');
  ...
  must((await page.locator('#v-rooms #backRooms').count()) === 1 && (await page.locator('#v-rooms .doorstep').count()) >= 1 && /Watch and read/.test(await page.locator('#wall').textContent()) && ((await page.locator('#wall a.link').count()) + (await page.locator('#wall .rd').count())) >= 1 && (await page.locator('#v-rooms .hint[data-hint]').count()) === 0, 'in a room: Corridor button, neighbouring doors, the wall shows this room, its cards, mistakes and something to read, and no hint');
  ...
  await page.waitForSelector('#wall .rd', { timeout: 10000 });
  must((await page.locator('#wall .rd').count()) === 3 && (await page.locator('#wall .rfree.mium').count()) === 1 && /PMT/.test(await page.locator('#wall .rd .rsite').first().textContent()) && (await page.locator('#wall [data-pinread]').count()) === 3 && !/bitesize\/search/.test(await page.locator('#wall').innerHTML()), 'the wall lists three checked pages under Read with site, free or freemium tag, why and Pin to desk, and no Bitesize search');
  await page.locator('#wall .rblock:has(.rd)').scrollIntoViewIfNeeded();
  await snap(page, '34-wall-reads', 'Watch and read on the wall: the checked video, then Read — three pages from trusted UK sites the Worker fetched and a model picked, each with its site, a free or freemium tag, one line on what it gives you, and Pin to desk');
  ...
  must(/data-act="signout"|id="signOut"/.test(await page.locator('#v-office').innerHTML()) && (await page.locator('#v-office .ccard').count()) === 10, 'the Office: Add or change courses, the account with Sign out, the ten courses with pins');
  ...
  await snap(page, '35-model-answer-levels', 'A levelled model answer on the Level 3 tab: the question, what the examiner is looking for, then the answer with what level 2 added in orange and what level 3 added in green; the tabs choose the level');
  await page.click('#v-rooms [data-mlevel$="|1"]'); await page.waitForTimeout(100);
  ...
  /* 16. AQA GCSE English Language: a skills course with no set texts, whose rooms carry their own passages */
  await page.click('#brand'); await page.waitForSelector('#qBar [data-course="AQA-8700"]');
  await page.click('#qBar [data-course="AQA-8700"]'); await page.waitForSelector('#v-rooms [data-open="AQA-8700|paper1"]');
  must((await page.locator('#v-rooms .pen').count()) === 0 && (await page.locator('#v-rooms .door').count()) === 9, 'corridor: all nine English Language rooms have their kit');
  await page.click('#v-rooms [data-open="AQA-8700|paper1"]');
  await page.waitForFunction(() => /Written and checked/.test(document.querySelector('#v-rooms').textContent), null, { timeout: 10000 });
  ...
  /* 18. AQA GCSE English Literature: the four set-text choices open six rooms, each with its checked kit; the Macbeth room prints its own short extracts */
  await page.click('#brand'); await page.waitForSelector('#qBar [data-course="AQA-8702"]');
  await page.click('#qBar [data-course="AQA-8702"]'); await page.waitForSelector('#v-rooms [data-open="AQA-8702|shakespeare-macbeth"]');
  must((await page.locator('#v-rooms .pen').count()) === 0 && (await page.locator('#v-rooms .door').count()) === 6, 'corridor: the chosen play, novel, modern text and cluster plus unseen poetry and skills make six English Literature rooms, every one with its kit');
  await page.click('#v-rooms [data-open="AQA-8702|shakespeare-macbeth"]');
  await page.waitForFunction(() => /Written and checked/.test(document.querySelector('#v-rooms').textContent), null, { timeout: 10000 });
  must(/The extracts this room works from/.test(await page.locator('#v-rooms').textContent()) && /Model paragraph — Level 6/.test(await page.locator('#v-rooms').textContent()) && (await page.locator('#v-rooms [data-lxrev]').count()) >= 4, 'Macbeth room: the checked kit prints its extracts, a Level 6 model paragraph and faded worked examples');
  await snap(page, '38-english-literature-room', 'AQA GCSE English Literature, Macbeth as a room: the checked kit lesson working from the room’s own short extracts, the Level 6 and Level 3 model paragraphs, and the provenance line');
  /* 19. AQA GCSE Geography: the three option choices open seventeen rooms, each with its checked kit; the Natural hazards room prints its own resources */
  await page.click('#brand'); await page.waitForSelector('#qBar [data-course="AQA-8035"]');
  await page.click('#qBar [data-course="AQA-8035"]'); await page.waitForSelector('#v-rooms [data-open="AQA-8035|3.1.1.1"]');
  must((await page.locator('#v-rooms .pen').count()) === 0 && (await page.locator('#v-rooms .door').count()) === 17, 'corridor: the chosen ecosystem, two landscapes and one resource plus the thirteen compulsory rooms make seventeen Geography rooms, every one with its kit');
  await page.click('#v-rooms [data-open="AQA-8035|3.1.1.1"]');
  await page.waitForFunction(() => /Written and checked/.test(document.querySelector('#v-rooms').textContent), null, { timeout: 10000 });
  must(/The resources written for this room/.test(await page.locator('#v-rooms').textContent()) && /Model paragraph — Level 3/.test(await page.locator('#v-rooms').textContent()) && (await page.locator('#v-rooms [data-lxrev]').count()) >= 4, 'Natural hazards room: the checked kit prints its resources, a Level 3 model paragraph and faded worked examples');
  await snap(page, '39-geography-room', 'AQA GCSE Geography, Natural hazards as a room: the checked kit lesson working from the room’s own figures, the Level 3 and Level 1 model paragraphs, and the provenance line');
  /* 20. AQA GCSE Mathematics (Higher tier): both tiers' rooms open, each with its checked kit and a Formulae sheet read against AQA's own insert */
  await page.click('#brand'); await page.waitForSelector('#qBar [data-course="AQA-8300"]');
  await page.click('#qBar [data-course="AQA-8300"]'); await page.waitForSelector('#v-rooms [data-open="AQA-8300|geometry-mensuration-higher"]');
  must((await page.locator('#v-rooms .pen').count()) === 0 && (await page.locator('#v-rooms .door').count()) === 26, 'corridor: the thirteen Foundation rooms and the thirteen Higher additions make twenty-six AQA Maths rooms, every one with its kit');
  await page.click('#v-rooms [data-open="AQA-8300|geometry-mensuration-higher"]');
  await page.waitForFunction(() => /Written and checked/.test(document.querySelector('#v-rooms').textContent), null, { timeout: 10000 });
  must(/Formulae sheet/.test(await page.locator('#v-rooms').textContent()) && /sine rule/i.test(await page.locator('#v-rooms').textContent()) && (await page.locator('#v-rooms [data-lxrev]').count()) >= 4, 'Higher mensuration room: the checked kit carries its Formulae sheet naming the sine rule as printed on the Higher insert, and faded worked examples');
  await snap(page, '40-aqa-maths-higher-room', 'AQA GCSE Mathematics (Higher tier), Mensuration — Higher additions: the checked kit lesson and its Formulae sheet read against AQA’s 2026 Higher insert');
  await page.context().close();
