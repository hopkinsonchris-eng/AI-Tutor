...
(async () => {
  ...
  /* ---------- videos for a room: searched, chosen by a model, verified, cached ---------- */
  {
    ...
    {const RD = readsMod.exports;
     ...
     let rd = await r.json();
     ok('RD2 the search is fenced to the room’s trusted sites plus its board, never BBC, and names the course and topic', searchSeen && searchSeen.tools[0].allowed_domains.includes('internetgeography.net') && searchSeen.tools[0].allowed_domains.includes('ocr.org.uk') && !searchSeen.tools[0].allowed_domains.includes('corbettmaths.com') && !searchSeen.tools[0].allowed_domains.some(d => /bbc|^www\./.test(d)) && searchSeen.tools[0].max_uses <= 5 && /H481/.test(searchSeen.messages[0].content) && /Coastal landscapes/.test(searchSeen.messages[0].content), JSON.stringify(searchSeen && searchSeen.tools));
