...
html=html.replace('/*__CONFIG__*/',()=>'const CONFIG='+JSON.stringify({tutorUrl})+';').replace('/*__CSS__*/',()=>css).replace('/*__CORE__*/',()=>core).replace('/*__GEN__*/',()=>gen).replace('/*__SPECS__*/',()=>'const SPECS='+JSON.stringify(byId)+';').replace('/*__AUTHORED__*/',()=>'const AUTHORED='+JSON.stringify(AUTHORED)+';');
...
console.log('built dist/index.html',(html.length/1024).toFixed(0)+' KB',tutorUrl?'· accounts via '+tutorUrl:'· no tutor route (legacy build)');
