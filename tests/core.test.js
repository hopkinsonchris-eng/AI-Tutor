...
const links=C.topicLinks(SPEC_H481,SPEC_H481.topics[3]);ok('G4 every topic gets derived video and reading search links, all https with a title and a kind',links.length>=2&&links.some(l=>/youtube\.com\/results\?search_query=.*Earth/.test(l.url))&&links.some(l=>/bbc\.co\.uk\/bitesize\/search/.test(l.url))&&links.every(l=>/^https:\/\//.test(l.url)&&l.t&&l.kind),JSON.stringify(links));
...
const l2=C.topicLinks(withHubs,topicWithLinks);ok('G4 the topic’s own links come first, then the course’s hub pages, then the searches',l2[0].url==='https://www.youtube.com/watch?v=abc'&&l2[1].url==='https://www.physicsandmathstutor.com/geography-revision/a-level-ocr/'&&l2.length===4,JSON.stringify(l2.map(l=>l.url)));
