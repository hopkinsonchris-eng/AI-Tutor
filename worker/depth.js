...
export const kitSchemas = {
  kit: S({
    lesson: S({ why: str, idea: arr(S({ h: str, t: str, code: str })), examples: arr(S({ title: str, steps: arr(str) })), check: arr(S({ q: str, a: str })) }),
    room: S({ facts: arr(str), questions: arr(S({ q: str, a: str, sol: str, m: int, d: int, hints: arr(str), codes: arr(str) })), exit: arr(S({ q: str, a: str })) }),
    cards: arr(S({ front: str, back: str, code: str })),
