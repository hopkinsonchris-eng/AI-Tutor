...
export const schemas = {
  outline: S({
    ...
    markConventions: S({ style: { type: 'string', enum: ['levels', 'points'] }, summary: str, commandWords: arr(S({ word: str, means: str })), essayShapes: arr(S({ marks: int, minutes: int, structure: str })), timingNote: str }, { required: ['style', 'summary', 'commandWords', 'essayShapes'] }),
...
Set "family" to the family that fits the document (the rules above are the prior; the document decides). Topics are the sections a student is examined on, in the document's order, with the document's own section numbers as ids (never an abbreviation); at most ${MAX_TOPICS}. Components' weights are the document's own percentages and must sum to 100 within rounding. paperCode is the paper's code only where the document prints one; otherwise leave it empty rather than composing one from the entry code. If the document gives only a whole-paper time and not a time per question, do not invent one: split that whole-paper time across its essayShapes in proportion to each shape's marks, and say so in markConventions.timingNote (state the paper's total minutes and marks and the split, the way a hand-built course does it) — a shape's minutes must always be present, but where they come from must never be hidden. Anything the document says students are given in the exam (a data sheet, periodic table, equation sheet, formulae booklet) is examined content: give each its own topic under the component that uses it, or one "Given in the exam" topic. Front matter that is not examined (aims, overview, administration) is not a topic.
