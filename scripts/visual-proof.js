...
(async () => {
  ...
  must((await page.locator('#v-campus .bldg[data-bldg]').count()) === 9 && (await page.locator('#v-campus [data-bldg-view="exam"]').count()) === 1 && (await page.locator('#v-campus [data-bldg-view="office"]').count()) === 1, 'campus: nine subject buildings, the Exam Hall and the Office');
  must((await page.locator('#qBar [data-course]').count()) === 9 && (await page.locator('#qBar [data-v="exam"]').count()) === 1 && (await page.locator('#qBar [data-v="office"]').count()) === 1 && !(await page.locator('nav.bottom').isVisible()), '1280px: the quick bar carries every course, the Exam Hall and the Office; no bottom bar');
  ...
  must(/data-act="signout"|id="signOut"/.test(await page.locator('#v-office').innerHTML()) && (await page.locator('#v-office .ccard').count()) === 9, 'the Office: Add or change courses, the account with Sign out, the nine courses with pins');
