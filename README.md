...
- **Reader.** A *Read this page* bar under the header and a 🔊 on every block of text. Each sentence is spoken by the browser's own voice (a British English voice is preferred) and highlighted inside its block as it is read; speed from 0.7× to 1.4×. *Line focus* dims everything except a window of one, three or five lines that follows the pointer or the reading; *Wider spacing* applies the WCAG text-spacing values; *Coach speaks* reads each reply aloud.
