...
async function tutor(u,o){const path=u.replace('https://tutor.studyplatform.co.uk','');const m=o.method||'GET';const body=o.body?JSON.parse(o.body):null;const auth=(o.headers||{}).Authorization||'';
  ...
  if(path==='/courses/build'&&m==='POST'){lastBuildReq=body;const id=body.board==='AQA'&&body.code==='7402'?'AQA-7402':body.board==='Pearson Edexcel'&&body.code==='1MA1'?'EDX-1MA1':'X';buildPolls=0;if(COURSE_STATES[id]&&COURSE_STATES[id].status==='published')return R(200,{status:'published',id});if(id==='EDX-1MA1')return R(202,{status:'review',id,build:{message:'Built, but the checks want a second look — it is being checked by Chris before it goes live.'}});return R(202,{status:'building',id,joined:false,build:{id,status:'building',stage:'Finding the specification',done:0,total:7,message:'Finding the official specification…'}});}
...
(async()=>{await sleep(40);const G=sb;G0=sb;
 ...
 $('crsSubject').value='Mathematics';await change({id:'crsSubject',value:'Mathematics'});$('crsBoard').value='EDX-1MA1';await click({id:'crsAdd'});
 ok('P4 a build the checks doubt tells the student it is being checked by Chris (criterion 5)',/being checked by Chris/.test(reg['v-setup'].innerHTML)&&!G.UI.setup.subjects.some(x=>x.specId==='EDX-1MA1'));
