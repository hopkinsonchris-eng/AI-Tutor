...
function papersMock(path,m,o,body){paperReqs.push({m,path,body});
  ...
  return R(404,{error:'no such papers route'});}let lastBuildReq=null;let buildPolls=0;let lastReq=null;const REQUESTS=[];
...
async function tutor(u,o){const path=u.replace('https://tutor.studyplatform.co.uk','');const m=o.method||'GET';const body=o.body&&typeof o.body==='string'?JSON.parse(o.body):null;const auth=(o.headers||{}).Authorization||'';
  ...
  if(path==='/courses/request'&&m==='POST'){lastReq=body;const id=body.board==='AQA'&&body.code==='7402'?'AQA-7402':body.board==='AQA'&&body.code==='8300'?'AQA-8300':'X';if(COURSE_STATES[id]&&COURSE_STATES[id].status==='published')return R(200,{status:'published',id});if(id==='AQA-8300')return R(202,{status:'review',id,message:'Built, and being checked by Chris before it goes live.'});REQUESTS.push(id);return R(202,{status:'requested',id,count:REQUESTS.length,message:'Sent to Chris. '+body.subject+' ('+body.board+') will be mapped for you, and you can add it once it is ready.'});}
...
(async()=>{await sleep(40);const G=sb;G0=sb;
 ...
 ok('P3 once Chris has mapped it, Add fetches it and adds it, still with no build started (criterion 2)',G.UI.setup.subjects.some(x=>x.specId==='AQA-7402')&&G.SPECS['AQA-7402']&&G.SPECS['AQA-7402'].subject==='Biology'&&!G.UI.build&&/data-su="AQA-7402" checked/.test(reg['v-setup'].innerHTML)&&lastBuildReq===null);
 ...
 G.S.hints={};/* a fresh student for the hints */G.go('campus');G.renderAll();const CV=()=>reg['v-campus'].innerHTML;const CR=()=>reg['v-rooms'].innerHTML;
 ...
 ok('N9 the first hint appears on the campus once and is remembered in state',/class="hint" data-hint="campus"/.test(CV())&&G.S.hints.campus===true);
 G.renderAll();ok('N9 re-rendering the same view keeps the hint up',/data-hint="campus"/.test(CV()));
 G.go('exam');G.go('campus');G.renderAll();ok('N9 leaving and coming back, the hint is gone for good',!/data-hint="campus"/.test(CV())&&JSON.parse(store['platform:state:v1:matthew']).hints.campus===true);
 ...
 ok('N9 the second hint appears in the corridor, once',/data-hint="corridor"/.test(CR())&&G.S.hints.corridor===true);
 ...
 ok('N9 the third hint appears in the room, once; all three are now in state',/data-hint="room"/.test(CR())&&G.S.hints.room===true&&G.S.hints.campus&&G.S.hints.corridor);
