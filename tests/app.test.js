...
(async()=>{await sleep(40);const G=sb;G0=sb;
 ...
 ok('X6 the yellow post-it carries the nudge, the pink one the latest note, the blue one today’s numbers',/data-postit="next"[\s\S]*?plan and/.test(svg())&&/data-postit="note"[\s\S]*?Ask Mr Hall/.test(svg())&&/data-postit="today"[\s\S]*?cards due/.test(svg())&&/Exam in \d+ days/.test(svg()));
 ok('X6 the yellow post-it opens the nudge’s station, the pink one the desktop',/data-station="essay" aria-label="Next step: Matthew, plan and mark one/.test(svg())&&/data-station="desktop" aria-label="Your note: Ask Mr Hall/.test(svg()));
 /* the plant */
 ...
 /* the planner */
 const dte=require('../src/core.js').daysToExam(G.S.setup,G.TODAY);ok('X8 the planner shows the exam countdown from the chosen year and today’s date',new RegExp(`${dte} days to go`).test(svg())&&/data-station="planner"/.test(svg())&&new RegExp(new Date(G.TODAY).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})).test(svg()));
 ...
 ok('X11 a room without a kit still draws the desk, with empty piles and no bookmark (criterion 11)',svg().length>2000&&/data-pile="1" data-sheets="0"/.test(svg())&&!/data-station="formulae"/.test(svg())&&/Pin a video here/.test(svg())&&!/data-postit="note"/.test(svg())&&/data-station="lesson"/.test(svg()));
